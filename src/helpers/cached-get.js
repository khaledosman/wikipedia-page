...
export function cachedGet (url, config, {
  ...
  showErrorMessage
} = { cacheKey: url, isOnlineFirst: false, dontShowErrorMessage: false, showErrorMessage: () => {} }) {
  ...
  if (isOnlineFirst) {
    ...
    // Get from cache first, but make the request anyway to update the cache in the background, or fallback to network
    return Promise.race([
